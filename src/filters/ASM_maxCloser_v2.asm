...
mov rcx,0; rcx <--- cantidad de filas ; el loop de afuera va a ciclar por las filas
;inc rcx; SERA ESTO ??????????? FIJATE!!! Si, era eso.

.ciclo_filas:
cmp rcx,r14
je .fin
push rcx; guardamos el indice de filas (0 <= rcx = i <= srch)
mov rcx,0; rcx <-- cantidad de columnas (j = srcw)
...
packuswb xmm0,xmm0   ; xmm0 = | 0 | 0 | 0 | 0 | basura | R | G | B | 0 | 0 | 0 | 0 | basura | R | G | B |
